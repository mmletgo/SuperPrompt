...
* **任务:**
  1. **分析与生成UI设计任务清单:** 调用 `sprint-prioritizer`，**输入宏观目标："根据UX线框图，创建高保真UI设计规范md文档和所有界面视觉稿html"。** `sprint-prioritizer` 必须读取 `designs/ux/wireframes.md`、`designs/ux/user-flows.md`、`docs/front-end-spec.md`、`docs/task_format_spec.md`，全面理解需求，并生成一个结构化的、按依赖关系和优先级排序的UI设计任务列表。任务清单需按以下任务类别顺序组织：1. 设计系统基础（调色板、字体、基础组件规范）；2. 核心UI组件设计；3. 页面布局和界面设计。
...
* **对任务的特殊要求:**
  * **高保真UI设计规范的产出是design-spec.md和对应的展示html，界面视觉稿的产出是html文件，除了有所有页面的视觉稿html外，还要有基础组件和核心组件规范的展示html**
...
* **目标:** 基于已批准的线框图和用户流程，高效、高质量地完成所有UI设计任务，创建高保真、可投入生产的设计系统和所有界面视觉稿。
* **智能体:** `ui-designer`
* **任务说明:** 作为`ui-designer` 按照任务清单逐步完成所有UI设计任务。

* **执行流程:**
  1. 作为`ui-designer`读取 `Worknotes/stage-4-ui-design.json`，获取所有状态为"pending"的任务清单。
  2. 按照任务的依赖关系，让`ui-designer`逐个执行任务：
     * 优先执行无依赖任务或所有依赖任务均已"completed"的任务
     * 将当前执行任务的状态更新为"in_progress"
     * 完成任务后，将状态更新为"completed"
     * 继续执行下一个符合条件的任务
  3. 重复步骤2，直到所有任务完成。

* **任务执行:**
  * **执行:** `ui-designer` 根据任务需求，读取 `designs/ux/` 中的文件和依赖任务的产出，使用 shadcn/ui 和 Tailwind CSS 的设计理念，创建高保真UI设计稿html和UI设计规范。**在执行任务时，如果发现系统中已存在与本次设计任务相关的文件，必须优先基于现有文件进行修改和完善，而不是从头开始重写。**
  * **状态更新:** 每个任务完成后，`ui-designer` 将 `Worknotes/stage-4-ui-design.json` 中对应任务的状态更新为"completed"。
...
* **对任务的特殊要求:**
  ...
  * UI设计规范: `ui-designer` 需要创建详细的设计规范文档。它必须包含：  
    ...
    * 排版：所有文本元素的字体选择、大小和粗细。  
    * 组件设计：对于`docs/front-end-spec.md`中标识的每个关键组件，提供视觉描述，列出其不同状态（默认、悬停、活动、禁用、屏蔽），并指定填充/边距。
    * 基础组件和核心组件的展示html必须在 `designs/ui/` 目录下，文件名必须是 `base-components.html` 和 `core-components.html`。
